...
public class LogGenerator {
    ...
    public static File generate(String fileName, int durationMinutes, int logsPerSecond) throws IOException {
    ...
    private static String generateRandomLogLine(LocalDateTime timestamp) {
        ...
        if (rand < 70) {
            ...
            return String.format("%s INFO  [trace_id=%s span_id=%s] Request processed successfully for user_%d", 
                tsString, traceId, spanId, RANDOM.nextInt(1000));
        } else if (rand < 90) {
            // DEBUG - Verbose (20%)
            return String.format("%s DEBUG [trace_id=%s span_id=%s] Database query took %d ms", 
                tsString, traceId, spanId, RANDOM.nextInt(50));
        } else if (rand < 95) {
            // WARN - Slow Query (5%)
            return String.format("%s WARN  [trace_id=%s span_id=%s] Slow query detected on table customers", 
                tsString, traceId, spanId);
        } else if (rand < 99) {
            // ERROR - Exception (4%)
            return String.format("%s ERROR [trace_id=%s span_id=%s] Internal Server Error\n" +
            ...
            // METRIC (1%)
            return String.format("%s METRIC cpu_usage=%d memory_usage=%d disk_io=%d", 
                tsString, RANDOM.nextInt(100), RANDOM.nextInt(4096) + 1024, RANDOM.nextInt(500));
        }
    }
}
